...
```{r plot_chunk, fig.width=8, fig.height=6, message=FALSE, warning=FALSE}
...
When searching the STRING database for gene interactions, there are a variety of variable that can be adjusted. These variable modifications have been included in DeSciDe's usage. First is the specification of the species of interest. The package defaults to search for human genes, but this can be changed by the user with argument `species =` as shown below:\
...
We can run just the STRING search to see the differences in the network produced by using full or physical network. Wiith the full network, we see many more interactions between genes:
